...
async function calculateFunction() {
    ...
    if (!contract) {
        ...
        performMockCalculation(selectedFunction, inputNum);
        return;
    }
...
function performMockCalculation(functionName, inputValue) {
    let solidityResult, rustResult;
    ...
    switch (functionName) {
        ...
        case 'ln':
            if (inputValue <= 0) {
                alert('Logarithm undefined for non-positive values');
                return;
            }
...
async function runMockComprehensiveTest(testValue) {
    ...
    for (const func of functions) {
        ...
        performMockCalculation(func, testValue);
